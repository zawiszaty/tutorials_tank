...
use App\Application\Command\Message\View\MessageViewCommand;
use App\Application\Query\Message\GetAll\GetAllCommand;
use App\Application\Query\Message\GetAllMessageSender\GetAllMessageSenderCommand;
...
class MessageController extends RestController
{
     ...
     * @NelmioSecurity(name="BearerUser")
     */
    public function getAllUserSender(Request $request): Response
    {
        $page = $request->get('page') ?? 1;
        $limit = $request->get('limit') ?? 10;
        $command = new GetAllMessageSenderCommand($page, $limit, $this->getUser()->getId());
        $model = $this->queryBus->handle($command);
     ...
     * @NelmioSecurity(name="BearerUser")
     */
    public function viewMessageAction(Request $request): Response
    {
        $messages = $request->request->get('messages');
        $command = new MessageViewCommand($messages);
