...
namespace App\Tests\Application\Command\User\UnBanned;
 ...
 */
class UnBannedUserHandlerTest extends ApplicationTestCase
{
    /**
     * @test
     *
    ...
    public function command_handler_must_fire_domain_event(): void
    {
        ...
        $command = new UnBannedUserCommand($userCreatedEvent->getId()->toString());
        $this->handle($command);
        /** @var DomainMessage[] $events */
        $events = $collector->popEvents();
        self::assertCount(1, $events);
        /** @var UserUnBann $userCreatedEvent */
        $userBannedEvent = $events[0]->getPayload();
        self::assertInstanceOf(UserUnBann::class, $userBannedEvent);
    }
}
